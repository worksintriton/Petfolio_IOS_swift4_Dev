...
class diagnosiViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        for i in 0..<self.diagno_dic_array.count{
            let dataval = self.diagno_dic_array[i] as! NSDictionary
            let diagnosis = dataval["diagnosis"] as? String ?? ""
            let id = dataval["_id"] as? String ?? ""
            if diagnosis == self.sear_diag[indexPath.row] {
                Servicefile.shared.doc_diag = self.sear_diag[indexPath.row]
                Servicefile.shared.doc_diag_id = id
            }
        }
