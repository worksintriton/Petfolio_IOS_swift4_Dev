...
class petloverEditandAddViewController: UIViewController, UITextFieldDelegate{
...
//        }
    ...
    func calladdpetdetails(){
        ...
        self.startAnimatingActivityIndicator()
        if Servicefile.shared.updateUserInterface() { AF.request(urldetails, method: .post, parameters: params
            , encoding: JSONEncoding.default).validate(statusCode: 200..<600).responseJSON { response in
                ...
                case .success:
                    ...
                    if Code == 200 {
                        ...
                        let vc = UIStoryboard.pet_edit_otherinfo_ViewController()
                        self.present(vc, animated: true, completion: nil)
                        self.stopAnimatingActivityIndicator()
                    }else{
                        self.stopAnimatingActivityIndicator()
